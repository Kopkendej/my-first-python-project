...
def jatek():
    futas = True
    valasztott = False  # Ez a változó követi, hogy történt-e már választás
    ...
    while futas:
        for esemeny in pygame.event.get():
            if esemeny.type == pygame.QUIT:
                futas = False
            elif esemeny.type == pygame.KEYDOWN and not valasztott:  # Csak akkor reagál, ha még nem történt választás
                if esemeny.key == pygame.K_1:  # A kunyhót választja
                    print("A játékos a kunyhót választotta.")
                    kunyho_ut(font, Ablak)
                    valasztott = True  # Választás lezárása
